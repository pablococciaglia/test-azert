...
export const AzertiumApp = () => {
    ...
    const handleBuscador = (e) =>{
        ...
        setmensaje(<><div className="spinner-grow text-primary" role="status"> </div><span>Espere...</span></>) 
        
        apiRequest(searchText)
        .then( data => { //manejo de la petición asincrona
            ...
            if( data.items.length === 0){setmensaje("No se ha encontrado ningún Framework con ese nombre")};// Manejo de la UI
            
        })
        .catch( err=> {

            console.log(err);
            setmensaje("El servidor no responde correctamente, por favor inténtelo más tarde"); // Manejo de la UI indicando que hubo falla en la petición al servidor

        }) 
    }

    useEffect(() => {// Manejo de la UI cuando el cuadro de dialogo está vacío aparece el mensaje inicial
